...
def generate_launch_description():
    ...
    # 自定义包
    pkg_config      = get_package_share_directory("robot_arm_moveit_config")
    bringup_config  = get_package_share_directory("robot_bringup")
    
    # 自定义路径
    urdf_path        = os.path.join(pkg_config, "config", "arm5dof.urdf.xacro")
    srdf_path        = os.path.join(pkg_config, "config", "arm5dof.srdf")
    ...
    bringup_config_path     = os.path.join(bringup_config, "config", "bringup.yaml") #给节点的参数
    ...
    node_rviz = Node(
        ...
        #给要运行的可执行文件传递命令行参数（对应到Rviz2来说，就是启动RViz2时传入的命令行参数）
